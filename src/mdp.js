...
export function createMDP(tagId) {
    ...
    const getDelay = function(rowIdx, colIdx) {
    ...
    const circles = svg
      ...
      .append("g")
        ...
        .attr("circle-row", function(d, j) { return j; }) // the row number available from DOM
      .selectAll("circle")
      .data(function(d){
        return d.coords;
      })
      ...
      .append("circle")
        ...
        .attr("fill", function (d,i) { return getFill(this.parentNode.getAttribute("circle-row"), i); })
        // Uncomment below if using the circleTransitions function instead
        .style("opacity", 0)
        .transition("displayCircleTransition")
        .duration(0)
        .delay(function (_,i) { return getDelay(this.parentNode.getAttribute("circle-row"), i); })
        .style("opacity", 1)
    ...
    let text = svg.append("g")
      ...
      .append("g")
        // h/t Zim https://stackoverflow.com/questions/38233003/d3-js-v4-how-to-access-parent-groups-datum-index
        .attr("circle-row", function(d, j) { return j; }) // make parent index (row number) available from DOM
      .selectAll("foreignObject")
      .data(function(d){
        return d.coords;
      })
      ...
      .append("xhtml:body")
        ...
        .attr("id", "nodeText")
        .style("opacity", 0)
        .transition("displayTextTransition")
        .duration(0)
        .delay(function (_,i) { return getDelay(this.parentNode.parentNode.getAttribute("circle-row"), i); })
        .style("opacity", 1)
    ...
    svg
     ...
     .join("line")
       ...
       .delay((d,i) => ((i - Math.floor(Math.max(0, i-1)/3)) * arrowSpeed))
       .duration(arrowSpeed)
       .attr("x2", d=>d[1][0])
       .attr("y2", d=>d[1][1])
       .attr("stroke-width", 1)
